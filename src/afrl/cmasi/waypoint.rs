...
impl PartialEq for Waypoint {
    fn eq(&self, _other: &Waypoint) -> bool {
        true
        && &self.number == &_other.number
        && &self.next_waypoint == &_other.next_waypoint
        && &self.speed == &_other.speed
        && &self.speed_type == &_other.speed_type
        && &self.climb_rate == &_other.climb_rate
        && &self.turn_type == &_other.turn_type
        && &self.vehicle_action_list == &_other.vehicle_action_list
        && &self.contingency_waypoint_a == &_other.contingency_waypoint_a
        && &self.contingency_waypoint_b == &_other.contingency_waypoint_b
        && &self.associated_tasks == &_other.associated_tasks

    }
}

impl LmcpSubscription for Waypoint {
    fn subscription() -> &'static str { "afrl.cmasi.Waypoint" }
}

impl Struct for Waypoint {
...
impl Lmcp for Waypoint {
    ...
    fn deser(buf: &[u8]) -> Result<(Waypoint, usize), Error> {
        let mut pos = 0;
        let (si, u) = StructInfo::deser(buf)?;
        pos += u;
        if si == Waypoint::struct_info() {
            let mut out: Waypoint = Default::default();
            {
...
pub trait WaypointT: Debug + Send + ::afrl::cmasi::location3d::Location3DT {
    fn as_afrl_cmasi_waypoint(&self) -> Option<&Waypoint> { None }
    fn as_mut_afrl_cmasi_waypoint(&mut self) -> Option<&mut Waypoint> { None }
    ...
    fn next_waypoint(&self) -> i64;
    fn next_waypoint_mut(&mut self) -> &mut i64;
    fn speed(&self) -> f32;
    ...
    fn turn_type(&self) -> ::afrl::cmasi::turn_type::TurnType;
    fn turn_type_mut(&mut self) -> &mut ::afrl::cmasi::turn_type::TurnType;
    ...
    fn contingency_waypoint_a(&self) -> i64;
    fn contingency_waypoint_a_mut(&mut self) -> &mut i64;
    fn contingency_waypoint_b(&self) -> i64;
    fn contingency_waypoint_b_mut(&mut self) -> &mut i64;
    fn associated_tasks(&self) -> &Vec<i64>;
    fn associated_tasks_mut(&mut self) -> &mut Vec<i64>;

}

impl Clone for Box<WaypointT> {
    fn clone(&self) -> Box<WaypointT> {
        if let Some(x) = WaypointT::as_afrl_cmasi_waypoint(self.as_ref()) {
            Box::new(x.clone())
        } else if let Some(x) = WaypointT::as_afrl_cmasi_path_waypoint(self.as_ref()) {
            Box::new(x.clone())
...
impl Default for Box<WaypointT> {
    fn default() -> Box<WaypointT> { Box::new(Waypoint::default()) }
}

impl PartialEq for Box<WaypointT> {
    fn eq(&self, other: &Box<WaypointT>) -> bool {
        if let (Some(x), Some(y)) =
            (WaypointT::as_afrl_cmasi_waypoint(self.as_ref()),
             WaypointT::as_afrl_cmasi_waypoint(other.as_ref())) {
                x == y
        } else if let (Some(x), Some(y)) =
            (WaypointT::as_afrl_cmasi_path_waypoint(self.as_ref()),
             WaypointT::as_afrl_cmasi_path_waypoint(other.as_ref())) {
                x == y
...
impl Lmcp for Box<WaypointT> {
    ...
    fn deser(buf: &[u8]) -> Result<(Box<WaypointT>, usize), Error> {
        let (si, _) = StructInfo::deser(buf)?;
        if si == Waypoint::struct_info() {
            let (x, readb) = Waypoint::deser(buf)?;
            Ok((Box::new(x), readb))
...
impl ::afrl::cmasi::location3d::Location3DT for Waypoint {
    fn as_afrl_cmasi_waypoint(&self) -> Option<&Waypoint> { Some(self) }
    fn as_mut_afrl_cmasi_waypoint(&mut self) -> Option<&mut Waypoint> { Some(self) }
...
pub mod tests {
    ...
    impl Arbitrary for Waypoint {
        fn arbitrary<G: Gen>(_g: &mut G) -> Waypoint {
            Waypoint {
    ...
    quickcheck! {
        fn serializes(x: Waypoint) -> Result<TestResult, Error> {
            use std::u16;
        ...
        fn roundtrips(x: Waypoint) -> Result<TestResult, Error> {
            use std::u16;
            if x.vehicle_action_list.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            if x.associated_tasks.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
