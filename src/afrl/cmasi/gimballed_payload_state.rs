...
#[repr(C)]
pub struct GimballedPayloadState {
    pub payload_id: i64,
...
impl PartialEq for GimballedPayloadState {
    fn eq(&self, _other: &GimballedPayloadState) -> bool {
        true
...
impl LmcpSubscription for GimballedPayloadState {
    fn subscription() -> &'static str { "afrl.cmasi.GimballedPayloadState" }
}

impl Struct for GimballedPayloadState {
...
impl Lmcp for GimballedPayloadState {
    ...
    fn deser(buf: &[u8]) -> Result<(GimballedPayloadState, usize), Error> {
        let mut pos = 0;
        let (si, u) = StructInfo::deser(buf)?;
        pos += u;
        if si == GimballedPayloadState::struct_info() {
            let mut out: GimballedPayloadState = Default::default();
            {
...
pub trait GimballedPayloadStateT: Debug + Send + ::afrl::cmasi::payload_state::PayloadStateT {
    fn as_afrl_cmasi_gimballed_payload_state(&self) -> Option<&GimballedPayloadState> { None }
    fn as_mut_afrl_cmasi_gimballed_payload_state(&mut self) -> Option<&mut GimballedPayloadState> { None }
...
impl Clone for Box<GimballedPayloadStateT> {
    fn clone(&self) -> Box<GimballedPayloadStateT> {
        if let Some(x) = GimballedPayloadStateT::as_afrl_cmasi_gimballed_payload_state(self.as_ref()) {
            Box::new(x.clone())
        } else if let Some(x) = GimballedPayloadStateT::as_afrl_cmasi_camera_state(self.as_ref()) {
            Box::new(x.clone())
...
impl Default for Box<GimballedPayloadStateT> {
    fn default() -> Box<GimballedPayloadStateT> { Box::new(GimballedPayloadState::default()) }
}

impl PartialEq for Box<GimballedPayloadStateT> {
    fn eq(&self, other: &Box<GimballedPayloadStateT>) -> bool {
        if let (Some(x), Some(y)) =
            (GimballedPayloadStateT::as_afrl_cmasi_gimballed_payload_state(self.as_ref()),
             GimballedPayloadStateT::as_afrl_cmasi_gimballed_payload_state(other.as_ref())) {
                x == y
        } else if let (Some(x), Some(y)) =
            (GimballedPayloadStateT::as_afrl_cmasi_camera_state(self.as_ref()),
             GimballedPayloadStateT::as_afrl_cmasi_camera_state(other.as_ref())) {
                x == y
...
impl Lmcp for Box<GimballedPayloadStateT> {
    fn ser(&self, buf: &mut[u8]) -> Result<usize, Error> {
        if let Some(x) = GimballedPayloadStateT::as_afrl_cmasi_gimballed_payload_state(self.as_ref()) {
            x.ser(buf)
        } else if let Some(x) = GimballedPayloadStateT::as_afrl_cmasi_camera_state(self.as_ref()) {
    ...
    fn deser(buf: &[u8]) -> Result<(Box<GimballedPayloadStateT>, usize), Error> {
        let (si, _) = StructInfo::deser(buf)?;
        if si == GimballedPayloadState::struct_info() {
            let (x, readb) = GimballedPayloadState::deser(buf)?;
            Ok((Box::new(x), readb))
...
impl ::afrl::cmasi::payload_state::PayloadStateT for GimballedPayloadState {
    fn as_afrl_cmasi_gimballed_payload_state(&self) -> Option<&GimballedPayloadState> { Some(self) }
    fn as_mut_afrl_cmasi_gimballed_payload_state(&mut self) -> Option<&mut GimballedPayloadState> { Some(self) }
...
impl GimballedPayloadStateT for GimballedPayloadState {
    fn as_afrl_cmasi_gimballed_payload_state(&self) -> Option<&GimballedPayloadState> { Some(self) }
    fn as_mut_afrl_cmasi_gimballed_payload_state(&mut self) -> Option<&mut GimballedPayloadState> { Some(self) }
...
pub mod tests {
    ...
    impl Arbitrary for GimballedPayloadState {
        fn arbitrary<G: Gen>(_g: &mut G) -> GimballedPayloadState {
    ...
    quickcheck! {
        fn serializes(x: GimballedPayloadState) -> Result<TestResult, Error> {
            use std::u16;
        ...
        fn roundtrips(x: GimballedPayloadState) -> Result<TestResult, Error> {
            use std::u16;
            if x.parameters.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }

            let mut buf: Vec<u8> = vec![0; x.size()];
            let sx = x.ser(&mut buf)?;
            let (y, sy) = GimballedPayloadState::deser(&buf)?;
