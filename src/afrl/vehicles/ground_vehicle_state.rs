...
impl PartialEq for GroundVehicleState {
    fn eq(&self, _other: &GroundVehicleState) -> bool {
        true

    }
}

impl LmcpSubscription for GroundVehicleState {
    fn subscription() -> &'static str { "afrl.vehicles.GroundVehicleState" }
}

impl Struct for GroundVehicleState {
...
impl Lmcp for GroundVehicleState {
    ...
    fn deser(buf: &[u8]) -> Result<(GroundVehicleState, usize), Error> {
        let mut pos = 0;
        let (si, u) = StructInfo::deser(buf)?;
        pos += u;
        if si == GroundVehicleState::struct_info() {
            let mut out: GroundVehicleState = Default::default();
            {
...
pub trait GroundVehicleStateT: Debug + Send + ::afrl::cmasi::entity_state::EntityStateT {
    fn as_afrl_vehicles_ground_vehicle_state(&self) -> Option<&GroundVehicleState> { None }
    fn as_mut_afrl_vehicles_ground_vehicle_state(&mut self) -> Option<&mut GroundVehicleState> { None }

}

impl Clone for Box<GroundVehicleStateT> {
    fn clone(&self) -> Box<GroundVehicleStateT> {
        if let Some(x) = GroundVehicleStateT::as_afrl_vehicles_ground_vehicle_state(self.as_ref()) {
            Box::new(x.clone())
...
impl Default for Box<GroundVehicleStateT> {
    fn default() -> Box<GroundVehicleStateT> { Box::new(GroundVehicleState::default()) }
}

impl PartialEq for Box<GroundVehicleStateT> {
    fn eq(&self, other: &Box<GroundVehicleStateT>) -> bool {
        if let (Some(x), Some(y)) =
            (GroundVehicleStateT::as_afrl_vehicles_ground_vehicle_state(self.as_ref()),
             GroundVehicleStateT::as_afrl_vehicles_ground_vehicle_state(other.as_ref())) {
                x == y
...
impl Lmcp for Box<GroundVehicleStateT> {
    fn ser(&self, buf: &mut[u8]) -> Result<usize, Error> {
        if let Some(x) = GroundVehicleStateT::as_afrl_vehicles_ground_vehicle_state(self.as_ref()) {
            x.ser(buf)
    ...
    fn deser(buf: &[u8]) -> Result<(Box<GroundVehicleStateT>, usize), Error> {
        let (si, _) = StructInfo::deser(buf)?;
        if si == GroundVehicleState::struct_info() {
            let (x, readb) = GroundVehicleState::deser(buf)?;
            Ok((Box::new(x), readb))
...
impl ::afrl::cmasi::entity_state::EntityStateT for GroundVehicleState {
    fn as_afrl_vehicles_ground_vehicle_state(&self) -> Option<&GroundVehicleState> { Some(self) }
    fn as_mut_afrl_vehicles_ground_vehicle_state(&mut self) -> Option<&mut GroundVehicleState> { Some(self) }
...
impl GroundVehicleStateT for GroundVehicleState {
    fn as_afrl_vehicles_ground_vehicle_state(&self) -> Option<&GroundVehicleState> { Some(self) }
    fn as_mut_afrl_vehicles_ground_vehicle_state(&mut self) -> Option<&mut GroundVehicleState> { Some(self) }
}
...
pub mod tests {
    ...
    impl Arbitrary for GroundVehicleState {
        fn arbitrary<G: Gen>(_g: &mut G) -> GroundVehicleState {
            GroundVehicleState {
    ...
    quickcheck! {
        fn serializes(x: GroundVehicleState) -> Result<TestResult, Error> {
            use std::u16;
        ...
        fn roundtrips(x: GroundVehicleState) -> Result<TestResult, Error> {
            use std::u16;
            if x.payload_state_list.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            if x.associated_tasks.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            if x.info.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            ...
            let mut buf: Vec<u8> = vec![0; x.size()];
            let sx = x.ser(&mut buf)?;
            let (y, sy) = GroundVehicleState::deser(&buf)?;
