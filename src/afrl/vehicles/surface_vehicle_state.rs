...
#[repr(C)]
pub struct SurfaceVehicleState {
    pub id: i64,
    pub u: f32,
    pub v: f32,
    pub w: f32,
...
impl PartialEq for SurfaceVehicleState {
    fn eq(&self, _other: &SurfaceVehicleState) -> bool {
        true
        && &self.bank_angle == &_other.bank_angle
        && &self.speed == &_other.speed

    }
}

impl LmcpSubscription for SurfaceVehicleState {
    fn subscription() -> &'static str { "afrl.vehicles.SurfaceVehicleState" }
}

impl Struct for SurfaceVehicleState {
...
impl Lmcp for SurfaceVehicleState {
    fn ser(&self, buf: &mut[u8]) -> Result<usize, Error> {
        ...
        {
            let r = get!(buf.get_mut(pos ..));
            let writeb: usize = self.bank_angle.ser(r)?;
            pos += writeb;
        }
        {
            let r = get!(buf.get_mut(pos ..));
    ...
    fn deser(buf: &[u8]) -> Result<(SurfaceVehicleState, usize), Error> {
        let mut pos = 0;
        let (si, u) = StructInfo::deser(buf)?;
        pos += u;
        if si == SurfaceVehicleState::struct_info() {
            let mut out: SurfaceVehicleState = Default::default();
            {
...
pub trait SurfaceVehicleStateT: Debug + Send + ::afrl::cmasi::entity_state::EntityStateT {
    fn as_afrl_vehicles_surface_vehicle_state(&self) -> Option<&SurfaceVehicleState> { None }
    fn as_mut_afrl_vehicles_surface_vehicle_state(&mut self) -> Option<&mut SurfaceVehicleState> { None }
    fn bank_angle(&self) -> f32;
    fn bank_angle_mut(&mut self) -> &mut f32;
    fn speed(&self) -> f32;
    fn speed_mut(&mut self) -> &mut f32;

}

impl Clone for Box<SurfaceVehicleStateT> {
    fn clone(&self) -> Box<SurfaceVehicleStateT> {
        if let Some(x) = SurfaceVehicleStateT::as_afrl_vehicles_surface_vehicle_state(self.as_ref()) {
            Box::new(x.clone())
...
impl Default for Box<SurfaceVehicleStateT> {
    fn default() -> Box<SurfaceVehicleStateT> { Box::new(SurfaceVehicleState::default()) }
}

impl PartialEq for Box<SurfaceVehicleStateT> {
    fn eq(&self, other: &Box<SurfaceVehicleStateT>) -> bool {
        if let (Some(x), Some(y)) =
            (SurfaceVehicleStateT::as_afrl_vehicles_surface_vehicle_state(self.as_ref()),
             SurfaceVehicleStateT::as_afrl_vehicles_surface_vehicle_state(other.as_ref())) {
                x == y
...
impl Lmcp for Box<SurfaceVehicleStateT> {
    fn ser(&self, buf: &mut[u8]) -> Result<usize, Error> {
        if let Some(x) = SurfaceVehicleStateT::as_afrl_vehicles_surface_vehicle_state(self.as_ref()) {
            x.ser(buf)
    ...
    fn deser(buf: &[u8]) -> Result<(Box<SurfaceVehicleStateT>, usize), Error> {
        let (si, _) = StructInfo::deser(buf)?;
        if si == SurfaceVehicleState::struct_info() {
            let (x, readb) = SurfaceVehicleState::deser(buf)?;
            Ok((Box::new(x), readb))
...
impl ::afrl::cmasi::entity_state::EntityStateT for SurfaceVehicleState {
    fn as_afrl_vehicles_surface_vehicle_state(&self) -> Option<&SurfaceVehicleState> { Some(self) }
    fn as_mut_afrl_vehicles_surface_vehicle_state(&mut self) -> Option<&mut SurfaceVehicleState> { Some(self) }
...
impl SurfaceVehicleStateT for SurfaceVehicleState {
    fn as_afrl_vehicles_surface_vehicle_state(&self) -> Option<&SurfaceVehicleState> { Some(self) }
    fn as_mut_afrl_vehicles_surface_vehicle_state(&mut self) -> Option<&mut SurfaceVehicleState> { Some(self) }
    fn bank_angle(&self) -> f32 { self.bank_angle }
    fn bank_angle_mut(&mut self) -> &mut f32 { &mut self.bank_angle }
...
pub mod tests {
    ...
    impl Arbitrary for SurfaceVehicleState {
        fn arbitrary<G: Gen>(_g: &mut G) -> SurfaceVehicleState {
            SurfaceVehicleState {
                ...
                bank_angle: Arbitrary::arbitrary(_g),
    ...
    quickcheck! {
        fn serializes(x: SurfaceVehicleState) -> Result<TestResult, Error> {
            use std::u16;
        ...
        fn roundtrips(x: SurfaceVehicleState) -> Result<TestResult, Error> {
            use std::u16;
            if x.payload_state_list.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            if x.associated_tasks.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            if x.info.len() > (u16::MAX as usize) { return Ok(TestResult::discard()); }
            ...
            let mut buf: Vec<u8> = vec![0; x.size()];
            let sx = x.ser(&mut buf)?;
            let (y, sy) = SurfaceVehicleState::deser(&buf)?;
